...
suite('PI-12: Expand All Command Tests', () => {
	...
	test('CollapseAll command should be registered', async () => {
		const extension = vscode.extensions.getExtension('douglashellinger.outline-eclipsed');
		await extension?.activate();

		const commands = await vscode.commands.getCommands(true);
		const hasCollapseAllCommand = commands.includes('outlineEclipsed.collapseAll');
		
		assert.ok(hasCollapseAllCommand, 'outlineEclipsed.collapseAll command should be registered');
	});
	...
	test('CollapseAll command should execute without errors', async () => {
		const extension = vscode.extensions.getExtension('douglashellinger.outline-eclipsed');
		await extension?.activate();
		...
		try {
			...
			assert.fail(`CollapseAll command should not throw error: ${error}`);
		}
	});
