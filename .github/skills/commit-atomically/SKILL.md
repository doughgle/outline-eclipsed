...
- Eliminate `any` type occurrences
...
- Use `type(scope): subject` with scope optional; start the subject with a verb.
- Add a blank line before the body when you include one.
- If the change is a workaround, link to the known issue.
- If the commit is a change to agent instructions, `copilot-instructions.md`, `AGENTS.md`, or `CLAUDE.md`, use `agents: ` as the type prefix.
